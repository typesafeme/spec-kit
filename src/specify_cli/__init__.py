...
def find_local_template(ai_assistant: str, script_type: str, search_dir: Path = None, verbose: bool = False) -> Optional[Path]:
    ...
    if verbose:
        for dir_path in search_dirs:
            if dir_path.exists():
                console.print(f"[cyan]Searching for local template:[/cyan] {pattern}*.zip in {dir_path}")
    ...
    if not all_matching_files:
        if verbose:
            ...
            console.print(f"[dim]Searched in: {', '.join(str(d) for d in search_dirs if d.exists())}[/dim]")
        return None
    ...
    if verbose:
        ...
        if len(all_matching_files) > 1:
            console.print(f"[dim]Other versions available: {', '.join(f.name for f, _, _ in all_matching_files[1:])}")
