...
if find_and_use_local_template "$FEATURE_DIR"; then
    ...
    # Failed to find/use local template
    >&2 echo "[specify] Error: Failed to use local template"
    >&2 echo "[specify] Please ensure a valid template zip file is in the current directory"
    exit 1
fi
